...
Because the Lunar soil is not produced by the erosion processes we are accustomed to on Earth the particulate matter is sharp and has reflectance properties unlike soils on Earth\todo{cite Hapke models}, which can pose a challenge to both human and automated perception systems. 
Notably, there is a significant opposition effect \cite{hapke1986bidirectional} in which the surface brightens and loses contrast when the view angle and illumination angle approach coincidence. \todo{Find an image reference for opposition effect}
...
We model expected [radiance/irradiance]\todo{Uland, fix this please} and attempt to approximate global illumination effects.   
