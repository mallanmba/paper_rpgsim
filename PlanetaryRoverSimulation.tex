...
Human operators must be able to perceive hazards and maintain adequate situational awareness in order to safely drive a rover, and the development of robust computer vision software requires images representative of those that would be received during them mission. 
...
Finally, the Lunar regolith that covers the surface of the Moon has unique reflective properties. 

Lunar regolith is formed through a microbombardment processes which pulvarizes rock into soil \cite{XXX}.  
Because the Lunar soil is not produced by the erosion processes we are accustomed to on Earth the particulate matter is sharp and has reflectance properties unlike soils on Earth\todo[inline]{cite Hapke models}, which can pose a challenge to both human and automated perception systems. 
Notably, there is a significant opposition effect \cite{hapke1986bidirectional} in which the surface brightens and loses contrast when the view angle and illumination angle approach coincidence. \todo[inline]{Find an image reference for opposition effect}
...
We developed a surface shader that models the reflective properties of Lunar regolith, and we draw the rover wheel tracks into the terrain. 
...
We use ephemeris models to accurately place the Sun and Earth in the sky for proposed mission landing dates. 
We model expected [radiance/irradiance]\todo[inline]{Uland, fix this please} and attempt to approximate global illumination effects.   
