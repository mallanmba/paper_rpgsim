...
After examining the options available in the industry, we chose to base our work on the robot simulator Gazebo~\cite{koenig2004design}\footnote{http://gazebosim.org}.
...
For similar reasons, we chose the widely used and also open source ROS (Robot Operating System) platform\footnote{http://www.ros.org} as the basis for our efforts in prototyping the vehicle control software~\cite{quigley2009ros}.
...
The position of the Sun and Earth relative to the moon is computed using the SPICE C toolkit provided by the Navigation and Ancillary Information Facility (NAIF) \footnote{https://naif.jpl.nasa.gov/naif/toolkit.html}.  
...
\begin{description}
\item[\textbf{Rover Kinematics}] The software that translates vehicle body motion commands into motor commands.  We modelled the Rover Kinematics module with the move\_base\footnote{http://wiki.ros.org/move\_base} module with additional explicit steering control.
...
\item[\textbf{Localization}]  The pose estimation solution which integrated wheel odometry, a star tracker, and the IMU.  We modelled the Localization module with ROS' robot\_localization\footnote{http://wiki.ros.org/robot\_localization} package.
...
\item[\textbf{Wheel, IMU, Gimbal, and Camera I/O}]  These modules responsible for low-level hardware interfaces.  This functionality was supplied by the ros\_control\footnote{http://wiki.ros.org/ros\_control} module.
...
\item[\textbf{Stereo Reconstruction}] Produces a metric reconstruction of the scene from the rover's stereo cameras.  We used OpenCV\footnote{http://wiki.ros.org/vision\_opencv} to perform Stereo Reconstruction.
...
Our approach generates 3D point clouds from successive stereo camera images, aligns them using the Point Cloud Library's (PCL)\footnote{http://pointclouds.org/} implementation of the Iterative Closest Point algorithm, and uses the transforms between point clouds to track changes in the rover's movements.  This module interacts with other software and sensors to estimate the rover's absolute pose.
...
A \texttt{rostest}\footnote{http://wiki.ros.org/rostest} script would spawn the rover at the base of each ramp and command it to drive 10 meters forward.
