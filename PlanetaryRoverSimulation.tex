...
Additionally, we needed a shadow solution that would support low illumination angles and long cast shadows as the reference mission scenarios have the sun at just 5 to 10 degrees above the horizon. 
...
Gazebo originally used the Ogre3D default shadow mapping algorithm which is a hybrid of Parallel Split Shadow Maps (PSSM)\cite{zhang2006parallel} and Light-Space Perspective Shadow Maps (LiSPSM)\cite{wimmer2004light}. PSSM is a form of cascaded shadow maps in which multiple depth maps are rendered in order to reduce shadow aliasing artifacts. LiSPSM is a perspective reparameterization technique that warps the shadow map in order to improve the distribution of shadow texels. 
...
These modifications mitigated the most severe artifacts, but there were still small features on the rover, such as the mast, that were not casting shadows consistently. 
...
Then we applied a 9-sample Poisson filter which smoothed the shadows even more and does a much better job of preserving small features.
...
This greatly simplified code associated with light placement. 
...
In order for this drift to be plausible in simulation, a wheel slip plug-in was developed and implemented for Gazebo that adjusts the slip compliance parameter based on a test results from two tests: RP single wheel drawbar pull test and RP Lunar mass equivalent unit test, MGRU (see figure \ref{fig:mgru}) in the SLOPElab facility at NASA Glenn Research Center (GRC) \cite{nasa2018slope}. 
