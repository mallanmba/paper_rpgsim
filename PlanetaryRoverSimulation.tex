...
However, the human perceptual system is unaccustomed to operating in the lunar environment, which caused problems for the Apollo astronauts in judging distances, estimating sizes, and tracking landmarks\cite{vaniman1991lunar}.  
...
In particular, the spatial distribution of cold-trapped volatiles is hypothesized to be governed by impact cratering with the top 0.5m \todo{top 0.5m of what?} being patchy at scales of 100m. 
The mixing time scale increases with depth (less frequent larger impacts). \todo{I think there is a link between mixing time scales by depth and increased lateral sampling which is not made for the non-planetary science reader.}
Consequently, increased mobility (greater range) reduces the depth requirement for sampling (i.e., drilling or excavation depth). \todo{Is the point that because the surface is patchy that we need to be able to move around?  We can't just pick one point and land there?}
...
These challenges\todo{which challenges?} have the potential to contribute to significant levels of operator workload for the team responsible for maneuvering the rover on the Lunar surface. 
These challenges may also impact the ease of acquiring and maintaining situation awareness, the ability to identify and handle contingencies, and ground control performance (rover driving, system monitoring, etc.)\todo{incomplete sentence}
...
We learned that with camera rotations of 50-degrees or less, stereo visual odometry yields more accurate results than the fused wheel and star tracker odometry we had been using (shown in Fig. \ref{fig:odom_error} ).
\begin{figure}[h!]
