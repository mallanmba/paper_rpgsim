...
Since replicating the lunar environment at scale for practice operations is unfeasible, we have constructed a simulation environment which presents representative visible characteristics, as well as approximations of the expected rover-terrain interactions.  

The Lunar polar environment provides short communications delays relative to Martian operations, which enable semi-interactive vehicle commanding.  
...
We built our simulation environment using open-source tools from the ROS ecosystem (\cref{sec:sim-approach}), which we also used to simulate the on- and off-board rover software (\cref{sec:flight-software}).  
...
The structure of the RP rover was modelled using ROS tools, and we developed models in the Gazebo simulation environment for modelling wheel slip (\cref{sec:vehicle-sim}).  

Although applicable to a broad range of scenarios, our lunar rover simulation was built initially to exercise and evaluate Resource Prospector's approach to lunar surface operations.
...
This paper focuses on the development and capabilities of the simulator itself and how it enabled evaluation of the driving task, but we touch on the science data interpretation task in a later section.
...
To fully enable the mission simulation we employed the planned flight control software.
This consisted of the robot driving software, VERVE, and the data display framework, WARP.
